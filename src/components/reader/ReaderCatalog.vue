 <template>
	...
	<div v-show="menu" class="reader-catalog">
      ...
      <mu-icon value="arrow_back_ios" @click='hidelist' style="vertical-align: middle;padding-left: 20px">
      </mu-icon>
				<p v-show="this.$store.state.isShowCatlog" class="catlog-reverse" @click="toChapter">
          章节跳跃
        </p>
		</div>
		<ul id="test" v-show="this.$store.state.isShowCatlog" class="book-catalog">
			<a @click="changeChapter(index)" href="javascript:"  v-for='(list,index) in catlog'>
        <li class="catalog-lists">
          {{list.title}}
        </li>
      </a>
		</ul>
